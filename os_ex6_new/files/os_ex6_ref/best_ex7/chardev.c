...
static ssize_t device_read(struct file *file, /* see include/linux/fs.h   */
               ...
               loff_t * offset)
{
    if (length <= 0 || put_user(Message[pos++], buffer) != 0)
      return -EFAULT;

    if (Message[pos] == '\0') pos = 0; //repeat name
...
{
    printk("device_write(%p,%d) - not supported\n", file, length);
...
static int simple_init()
{
    ...
    //init message
    printk("#Initializing message to be %s...", MY_NAME);
    strcpy(Message, MY_NAME);
    pos = 0;
