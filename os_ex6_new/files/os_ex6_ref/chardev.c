...
static int loc;		/* to store the location : where we are in the buffer */
               ...
               loff_t * offset)
{
    ...
    if(put_user(Message[loc], buffer) != 0){
	printk("Error : put_user() failed");
	return -EFAULT;
    }
    printk("device read : %c\n", Message[loc]);
    loc = (loc + 1) % strlen(Message);

    return 1;	//sucess in reading one char from Message
...
{
    printk("device_write - operation not supported...\n");
...
static int simple_init(void)
{
    ...
    loc = 0;	/* initializing the loc parameter to start of Message*/
    strcpy(Message, "Itzik Kabessa");	/* setting buffer to my name */
